...
export default function MdevPage(ctx) {
  ...
  const userIsSystemAdmin = ctx.session.user.system_admin == true;
  ...
  const result = locals.filter((local) => {
    if (!userIsSystemAdmin)
      return local.institution_id === ctx.session.user.institution_id;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const token = tokenService.get();
        const response = await HttpClient(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/mdevs/${parameter}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
